...
Python was first released in 1990.  This programming language has a large standard library.  The Python Package Index, called PyPI, the official repository for 3rd party Python software, has >130K packages, which have a wide range of functionality.  
...
What makes PyTorch so versatile and powerful is that it is able to tap into all the functionality of Python. Because Python is used widely, its syntax carries over into PyTorch, which makes the coding more familiar.  Hence, it is faster to learn and use.
