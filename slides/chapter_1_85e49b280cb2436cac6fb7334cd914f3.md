...
`@script`
PyTorch has a multi-dimensional array library with GPU support.  GPU support means it can provide greater data processing capability.  This is especially helpful with mathematically-intensive tasks such as deep learning.  This is an 

PyTorch also has packages that making deep learning possible such as automatic differentiation, gradient based optimization and utility packages for loading in data and pre-trained models.
...
`@script`
Just as Numpy provides an ndarray object, the equivalent in PyTorch is called a Tensor object.  There are different tensor types.  Some examples of types include:  float, double tensor, long tensor

The default tensor type is "float"

Users can perform 200+ operations including linear algebra and indexing.

And, this library in PyTorch has fast acceleration on NVIDIA GPUs.
...
Let's create an empty 2-d Numpy array
```
np.random.seed(0)
